...
export interface InfoOptions {
  ...
  json?: boolean;
  /** Include the verbatim GitHub API object and full README in --json. */
  full?: boolean;
}
...
export async function infoCommand(repoArg: string, opts: InfoOptions): Promise<void> {
  ...
  if (opts.json) {
    const { raw, enrichment, ...rest } = record;
    const out = opts.full
      ? record
      : {
          ...rest,
          enrichment: enrichment
            ? { ...enrichment, readme: enrichment.readme ? `[${enrichment.readme.length} chars — use --full]` : null }
            : null,
        };
